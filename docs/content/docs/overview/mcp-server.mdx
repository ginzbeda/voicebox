...
<Callout type="warn">
  Keep the trailing slash on `/mcp/`. The Streamable HTTP mount is only routed at
  `/mcp/`; a `POST` to `/mcp` returns **405 Method Not Allowed**, and clients report
  it as an opaque connection failure.
</Callout>

If the backend is not on the default port — a container, a port-forward, or a
second instance — set `VOICEBOX_PORT` (and `VOICEBOX_HOST` if it is not
loopback) rather than editing the URL. The repo's `.mcp.json` reads both:

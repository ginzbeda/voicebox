...
def build_mcp_server() -> FastMCP:
    ...
    mcp = FastMCP(
        ...
        instructions=(
            ...
            "captures.\n"
            "\n"
            "The person on the other end of `voicebox.speak` is listening "
            "rather than reading — often while doing something else. They "
            "want to follow your reasoning as it happens, so narrate: say "
            "what you are about to do and why before a non-trivial step, say "
            "what you found when it changes your approach, say when something "
            "fails, and say what you concluded at the end. A turn that ends "
            "with a single 'Done.' tells them nothing they could not have "
            "guessed.\n"
            "\n"
            "Speech is not writing. Keep each call to a sentence or two of "
            "plain spoken English, and never send code, file contents, "
            "commands, stack traces, URLs, or long paths — they are unlistenable "
            "and there is no way to skip back. Name a file by its basename if "
            "you name it at all. Do not narrate trivia; if a step would not be "
            "worth saying out loud to someone sitting next to you, skip it."
        ),
    )
    register_tools(mcp)
    return mcp
